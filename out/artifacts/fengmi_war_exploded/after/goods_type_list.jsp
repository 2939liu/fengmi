    ...
    function deleteConfirm(id, name) {
        ...
        confirm.show('溫馨提示', '您确定要刪除[' + name + ']商品类型吗？', {
            '确定': {
                'primary': true,
                'callback': function () {
                    location.href = "${pageContext.request.contextPath}/TypeDeleteServlet?id=" + id;
                }
            }
        });
    }
    ...
    <form id="deleteFrom" action="${pageContext.request.contextPath}/goods/deleteBatch" method="post">
        <table style="text-align: center;margin-right:10px;width:99%"
            ...
            <tr>
                ...
                <td><a href="<%=path%>/TypeDeleteServlet?id=${GoodsType.id}" onclick="if(confirm('是否确定删除'+'${GoodsType.typename}'+'？')==false)return false">删除</a></td> --%>
...
<span class="glyphicon glyphicon-edit"  color="#5BC0DE"></span></a></td>
                      ...
                      href="<%=path%>/TypeDeleteServlet?id=${goodsType.id}" onclick="if(confirm('是否确定删除'+'${goodsType.typeName}'+'？')==false)return false" ></a></td>
