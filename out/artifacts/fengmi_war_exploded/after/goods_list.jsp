...
<%
	...
	List<GoodsType> typeList = gt.seelectGoodsType(1);
...
<script>
	...
	function showAdd(){
		location.href="./after/add_goods.jsp";
	}
	function checkAll(){
		var goodsids=document.getElementsByName("goodsid");
		if(allnum == 0){
	...
	function delectAll(){
		if(confirm("确定删除吗？")){
			...
			window.location.href="<%=path%>/DeletGoodsServlet?goodsid="+ids;
		}
		...
		<table style="text-align: center; margin-right: 10px; width: 99%"
			...
			<c:forEach items="${list}" var="goods" varStatus="obj">
				<tr>
					...
					<td>
						<img
								src="http://localhost:8080/images/${goods.imgPath}"
